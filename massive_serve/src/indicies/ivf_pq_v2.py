...
try:
    from massive_serve.api.serve import rerank_encoder as RERANK_ENCODER
except Exception:
    RERANK_ENCODER = None
try:
...
# Set high-performance defaults (can still be overridden by env)
try:
    _cpu = os.cpu_count() or 1
    _blas_threads = str(_cpu)
    os.environ["OMP_NUM_THREADS"] = os.environ.get("OMP_NUM_THREADS", _blas_threads)
    os.environ["MKL_NUM_THREADS"] = os.environ.get("MKL_NUM_THREADS", _blas_threads)
    os.environ["OPENBLAS_NUM_THREADS"] = os.environ.get("OPENBLAS_NUM_THREADS", _blas_threads)
    os.environ["NUMEXPR_NUM_THREADS"] = os.environ.get("NUMEXPR_NUM_THREADS", _blas_threads)
    try:
        _faiss_threads = int(os.environ.get("FAISS_NUM_THREADS", str(_cpu)))
...
class IVFPQIndexer(object):
    ...
    def get_retrieved_passages(self, indices, raw_query=None):
        ...
        default_read_threads = str((os.cpu_count() or 1) * 4)
        max_workers_env = os.environ.get("FAISS_READ_THREADS", default_read_threads)
        try:
    ...
    def search(self, raw_query, query_embs, k, nprobe, expand_index_id=None, expand_offset=1, exact_rerank=False, diverse_rerank=False, lambda_val=0.5,
        ...
        for attempt in range(len(K_ranges)):
            ...
            for i, raw_passages in enumerate(all_raw_passages):
                # Match DS-Old behavior: apply min_words filter before attaching scores and reranking
                try:
                    if min_words is not None:
                        threshold = int(min_words)
                        if threshold > 0:
                            raw_passages = [p for p in raw_passages if isinstance(p.get("text"), str) and len((p.get("text") or "").strip().split()) >= threshold]
                except Exception:
                    pass

                # Attach FAISS retrieval scores so diverse rerank can use them when exact scores are absent
                try:
                ...
                if exact_rerank:
                    try:
                        # Prefer dedicated rerank encoder (e.g., GritLM); fallback to query_encoder.
                        _enc = RERANK_ENCODER or query_encoder
                        try:
                            _enc_name = getattr(_enc, 'model_id', None) or getattr(_enc, 'name', None) or type(_enc).__name__
                        except Exception:
                            _enc_name = str(type(_enc))
                        print(f"[ENCODER] ExactRerank using {_enc_name}")
                        raw_passages = exact_rerank_topk(raw_passages, _enc)
                ...
                if diverse_rerank:
                    try:
                        ...
                        try:
                            ...
                            _enc_name = str(type(_enc))
                        print(f"[ENCODER] DiverseRerank using {_enc_name} (lambda={lambda_val})")
                        raw_passages = diverse_rerank_topk(raw_passages, _enc, lambda_val=lambda_val)
